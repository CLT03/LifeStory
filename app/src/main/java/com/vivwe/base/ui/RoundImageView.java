...
public class RoundImageView extends AppCompatImageView {
    ...
    public RoundImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (attr) {
//                case R.styleable.RoundImageView_titleText:
//                    if(a.getString(attr).length() == 0){
//                        mTitleText = " ";
//                    } else {
//                        mTitleText = a.getString(attr);
//                    }
//                    break;
//                case R.styleable.RoundImageView_titleTextColor:
//                    // 默认颜色设置为黑色
//                    mTitleTextColor = a.getColor(attr, Color.BLACK);
//                    break;
//                case R.styleable.RoundImageView_titleTextSize:
//                    // 默认设置为16sp，TypeValue也可以把sp转化为px
//                    mTitleTextSize = a.getDimensionPixelSize(attr, (int) TypedValue.applyDimension(
//                            TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));
//                    break;
                case R.styleable.RoundImageView_backgroud:
                    mTitleText = "";
                    //默认为白色
                    background = a.getColor(attr, Color.WHITE);
                    break;
                case R.styleable.RoundImageView_mCornerSize:
                    //默认圆角为0
                    mCornerSize = a.getInteger(attr, 20);
                    break;
                case R.styleable.RoundImageView_paddingSize:
                    paddingSize = a.getInteger(attr, 0);
                    break;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        RectF rec = new RectF(paddingSize, paddingSize, getMeasuredWidth() - paddingSize, getMeasuredHeight() - paddingSize);
