package com.mbs.sdk.ui.listview.pulllistview;
...
//    private boolean showRefreshResultEnable; // 是否显示刷新加载结果
    ...
    // 是否启用下拉刷新
    private boolean refreshIsEnable = false;
    // 是否启用上拉加载更多
    private boolean loadMoreIsEnable = false;
    ...
    private void init(Context context) {
        ...
        refreshView = LayoutInflater.from(context).inflate(R.layout.zl_listview_refresh_head, this, false);
        loadMoreView = LayoutInflater.from(context).inflate(R.layout.zl_lsitview_load_foot, this, false);
        addView(refreshView);
...
//                        mResourceConfig.getImageResIds()[1]);
                Drawable drawable = getContext().getResources().getDrawable(mResourceConfig.getImageResIds()[1]);
                int height = (int)getContext().getResources().getDimension(R.dimen.x20);
                drawable.setBounds(1,1,height,height);
                refreshHintText.setCompoundDrawables(drawable, null, null, null);
                refreshHintText.setCompoundDrawablePadding(dp2px(10));
            } else { // 刷新失败
                refreshHintText.setText(mResourceConfig.getTextResIds()[4]);
                Drawable drawable = getContext().getResources().getDrawable(mResourceConfig.getImageResIds()[2]);
                int height = (int)getContext().getResources().getDimension(R.dimen.x20);
                drawable.setBounds(1,1,height,height);
                refreshHintText.setCompoundDrawables(drawable, null, null, null);
                refreshHintText.setCompoundDrawablePadding(dp2px(10));
        ...
        if (pullDownY > 0) {
            ...
            }, refreshIsEnable ? 1000 : 0); // 判断刷新结果停留1秒
        } else {
            changeStage(DONE);
            hide();
        }
    ...
    public void loadMoreFinish(boolean isSuccess) {
        ...
        if (loadMoreIsEnable) {
            if (isSuccess) { // 加载更多成功
                ...
                Drawable drawable = getContext().getResources().getDrawable(mResourceConfig.getImageResIds()[3]);
                int height = (int)getContext().getResources().getDimension(R.dimen.x20);
                drawable.setBounds(1,1,height,height);
                loadHintText.setCompoundDrawables(drawable, null, null, null);
                loadHintText.setCompoundDrawablePadding(dp2px(10));
            } else { // 加载更多失败
                loadHintText.setText(mResourceConfig.getTextResIds()[9]);
                Drawable drawable = getContext().getResources().getDrawable(mResourceConfig.getImageResIds()[4]);
                int height = (int)getContext().getResources().getDimension(R.dimen.x20);
                drawable.setBounds(1,1,height,height);
                loadHintText.setCompoundDrawables(drawable, null, null, null);
                loadHintText.setCompoundDrawablePadding(dp2px(10));
        ...
        if (pullUpY < 0) {
            ...
            },loadMoreIsEnable ? 1000 : 0); // 判断刷新结果停留1秒
        } else {
            changeStage(DONE);
            hide();
        }
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (stage == RELEASE_TO_REFRESH) {
                    ...
                    // 刷新操作
                    if (mOnRefreshListener != null && refreshIsEnable) {
                    ...
                    changeStage(LOADING);
                    // 加载操作
                    if (mOnRefreshListener != null && loadMoreIsEnable) {
    ...
    public boolean isRefreshIsEnable() {
        return refreshIsEnable;
    }

    /***
     * 设置是否启用下拉刷新
     * @param refreshIsEnable
     */
    public void setRefreshIsEnable(boolean refreshIsEnable) {
        this.refreshIsEnable = refreshIsEnable;
        refreshView.setVisibility(refreshIsEnable ? View.VISIBLE : View.INVISIBLE);
    }

    public boolean isLoadMoreIsEnable() {
        return loadMoreIsEnable;
    }

    /***
     * 设置是否启用上拉加载更多
     * @param loadMoreIsEnable
     */
    public void setLoadMoreIsEnable(boolean loadMoreIsEnable) {
        this.loadMoreIsEnable = loadMoreIsEnable;
        loadMoreView.setVisibility(loadMoreIsEnable ? View.VISIBLE : View.INVISIBLE);
    }
...
//    }
    ...
    public interface OnRefreshListener {
         ...
         * 下拉刷新
         */
        abstract void onRefresh(PullToRefreshLayout pullToRefreshLayout);

        /**
         * 上拉加载
         */
        abstract void onLoadMore(PullToRefreshLayout pullToRefreshLayout);
    }
}
